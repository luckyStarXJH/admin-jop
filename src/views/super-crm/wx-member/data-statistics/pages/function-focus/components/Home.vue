    ...
    <el-table
      ...
      <el-table-column
        ...
        <template slot-scope="scope">
          <el-link @click="focusItemDetails(computedProp(item))" :underline="false">{{ scope.row[computedProp(item)] }}</el-link>
        </template>
      </el-table-column>
...
export default class FunctionFocusHome extends Vue {
  private formInline = {
    time: ''
  };
  private tableData = [];
  ...
  private getTableSumActivity() {
    ...
    getTimeActivityByGroup(params).then((res: any) => {
      ...
      this.chartData.columns.unshift('time');
      this.chartData.rows = res.data || [];
    })
  }

  private focusItemDetails(item: string) {
    this.$router.push({
      path: '/super-crm/wx-member/data-statistics/function-focus/item',
      query: {
        api: item
      }
    })
