...
export default {
  ...
  methods: {
    ...
    showQRURL(item) {
      ...
      this.Linkvalue = `${process.env.VUE_APP_BASE_API}/wx-interface-web/wx/auth?companyKey=${user.serverUser.account.companyKey}&shareId=${data.id}&childPage=flop&page=wx_game`
      const params = {
    ...
    toPage(page, item) {
      ...
      switch (page) {
        case 'set':
          this.$router.push({
            path: '/super-mkt/activity-set/activity-type/playing-card/edit',
            query: {
        ...
        case 'data':
          this.$router.push({
            path: '/super-mkt/activity-set/activity-type/playing-card/data',
            query: {
              id: item.id,
              jmpId: item.jmpId,
              name: item.activityName
    ...
    toAddPage() {
      this.$router.push(
        '/super-mkt/activity-set/activity-type/playing-card/add'
      )
    },
    // 活动设置
